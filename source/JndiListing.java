...
public class JndiListing extends T3Connection {
	...
	private boolean loadJndi(boolean printGoodNews) {
		...
		if (this.isConnected() == true) {
			...
			myLogger.severe("Impossible to list JNDI because we can't establish a connection: "
					+ this.getLastConnectionErrorDescription());
			return false;
		}
	}
	...
	private void listContext(Context ctx, String indent, int pos, boolean printGoodNews, String appellant) {
		String className = "";
		String name = "";
		myLogger.finest("Run the fonction listContext(). Pos=" + pos + ", lastPath=" + this.lastPath + ", paths="
				+ this.getPrintableJndiList());
		try {
			if (pos == 0) {
				this.lastPath.clear();
			}
			...
			while (list.hasMore()) {
				...
				name = item.getName();
				if (printGoodNews == true) {
					printGoodNews(indent + className + " " + name);
				}
	...
	private void deleteLastElementOfCurrentPath() {
		...
		myLogger.finest(
				"Deleting the element " + lastPos + " ('" + this.lastPath.get(lastPos) + "') of " + this.lastPath);
